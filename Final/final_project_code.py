...
##note1: BadgerMatrix.txt was edited to work with code ('Badger' was removed from top left)
##note2: BadgerInfo.txt was edited to work with code (blank line removed from end)

def main():
	##TEXTFILE is the file that contains the data on the network
	nodes, edges, matrix, neighbors = convert_file('TEXTFILE.txt', True)
	edge_weights = edge_weight_finder(nodes, matrix)
	##NUM_OF_SIMS is the number of desires simulations
	clusterings = d_freq_finder('NUM_OF_SIMS', nodes, nodes, matrix, neighbors, edge_weights)
